function sortTask(tasks, sort) {
    ...
    for (let i = 1; i < tasks.length && sortTimeCreated !== undefined; ++i) {
        const key = tasks[i];
        let j = i - 1;
        while (j >= 0 && compareValue(sortTaskName, key.taskTimeCreated.slice(0, 10), tasks[j].taskTimeCreated.slice(0, 10))) {
            tasks[j + 1] = tasks[j];
            j--;
        }
        tasks[j + 1] = key;
    ...
    for (let i = 1; i < tasks.length && sortTimeDeadline !== undefined; ++i) {
        const key = tasks[i];
        let j = i - 1;
        while (j >= 0 && compareValue(sortTaskName, key.taskTimeDeadline.slice(0, 10), tasks[j].taskTimeDeadline.slice(0, 10))) {
            tasks[j + 1] = tasks[j];
            j--;
        }
        tasks[j + 1] = key;
